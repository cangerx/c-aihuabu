...
- `proxy/` 是当前同域 AI 请求代理，只做 HTTP 转发，不按业务后端分层。
- 如果后续新增账号、任务、素材等独立业务后端，后端使用 Go + Gin + GORM。
...
- 当前 AI API Key 存在浏览器本地，浏览器直连时由前端直接请求 OpenAI 兼容接口，同域代理开启时经 `/api/proxy` 转发；涉及安全说明时要写清楚。
