...
export const defaultConfig: AiConfig = {
    ...
    systemPrompt: "",
    models: ["default::gpt-image-2", "default::grok-imagine-image-quality", "default::grok-imagine-image", "default::grok-imagine-image-lite", "default::grok-imagine-video", "default::grok-imagine-video-1.5", "default::gpt-5.5", "default::gpt-4o-mini-tts"],
    imageModels: ["default::gpt-image-2", "default::grok-imagine-image-quality", "default::grok-imagine-image", "default::grok-imagine-image-lite"],
...
function isImageModelName(model: string) {
    const value = modelOptionName(model).toLowerCase();
    return !isVideoModelName(model) && !isAudioModelName(model) && (value.includes("seedream") || value.includes("gpt-image") || value.includes("image") || value.includes("dall-e") || value.includes("dalle") || value.includes("imagen") || value.includes("gemini") && value.includes("image") || value.includes("flux") || value.includes("sdxl") || value.includes("stable-diffusion") || value.includes("midjourney") || value.includes("banana"));
}
...
export function buildProxiedUrl(targetUrl: string, enabled = useConfigStore.getState().config.aiProxyEnabled) {
    ...
    if (!/^https?:\/\//i.test(url)) return targetUrl;
