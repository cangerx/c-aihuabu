...
import { createRechargeOrder, getMember, getMemberLedger, getPaymentOptions, getPointPackages } from "@/services/api/membership";
...
export default function AccountPage() {
    ...
    useEffect(() => { const methods = paymentOptions.data?.methods; if (methods?.length && !methods.includes(payMethod)) setPayMethod(methods[0]); }, [payMethod, paymentOptions.data?.methods]);
    if (!member) return <Navigate to="/auth?redirect=/account" replace />;
    const enabledMethods = paymentOptions.data?.methods || [];
    const startPay = async (packageId: string) => { if (!enabledMethods.includes(payMethod)) { message.warning("当前没有可用的支付方式"); return; } setPaying(true); try { setQr(await createRechargeOrder(packageId, payMethod)); } catch (error) { message.error(error instanceof Error ? error.message : "创建支付订单失败"); } finally { setPaying(false); } };
    return <main className="min-h-full overflow-y-auto bg-[#f5f5f3] px-5 py-8 dark:bg-[#101112]"><div className="mx-auto max-w-5xl">
        ...
        <section className="mt-6"><div className="mb-3 flex items-center gap-2 text-lg font-semibold"><UserRound className="size-5" />积分套餐</div><div className="mb-4 flex items-center gap-3 text-sm text-stone-500">支付方式{enabledMethods.length ? <Segmented value={payMethod} options={enabledMethods.map((value) => ({ label: value === "WECHAT" ? "微信" : "支付宝", value }))} onChange={(value) => setPayMethod(value as "WECHAT" | "ALIPAY")} /> : <Tag>暂未开放在线支付</Tag>}</div><Row gutter={[16, 16]}>{packages.data?.length ? packages.data.map((item) => <Col key={item.id} xs={24} sm={12} lg={8}><Card bordered={false} className="!rounded-2xl" title={item.name} extra={<Tag color="gold">{item.points} 积分</Tag>}><div className="flex items-center justify-between"><span className="text-2xl font-semibold">¥{(item.priceCent / 100).toFixed(2)}</span><Button type="primary" loading={paying} disabled={!enabledMethods.length} onClick={() => void startPay(item.id)}>购买积分</Button></div></Card></Col>) : <Col span={24}><Empty description="暂无可购买套餐" /></Col>}</Row></section>
