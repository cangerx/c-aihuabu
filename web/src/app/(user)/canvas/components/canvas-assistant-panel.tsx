...
export function CanvasAssistantPanel({ nodes, selectedNodeIds, snapshot, sessions, activeSessionId, onSelectNodeIds, onSessionsChange, onApplyOps, canUndoOps, onUndoOps, onPasteImage, agentMode, onAgentModeChange, closing, onCollapse }: CanvasAssistantPanelProps) {
    ...
    const width = useCanvasPanelStore((state) => state.agentPanelWidth);
    const setWidth = useCanvasPanelStore((state) => state.setAgentPanelWidth);
    ...
    const startResize = () => {
        const move = (event: MouseEvent) => setWidth(Math.min(AGENT_PANEL_MAX_WIDTH, Math.max(AGENT_PANEL_MIN_WIDTH, window.innerWidth - event.clientX)));
        const stop = () => {
            setResizing(false);
...
function describeCanvasSnapshot(snapshot: CanvasAgentSnapshot) {
    ...
    return `当前画布有 ${snapshot.nodes.length} 个节点、${snapshot.connections.length} 条连线。文本 ${counts[CanvasNodeType.Text] || 0} 个，图片 ${counts[CanvasNodeType.Image] || 0} 个，生成配置 ${counts[CanvasNodeType.Config] || 0} 个，视频 ${counts[CanvasNodeType.Video] || 0} 个，音频 ${counts[CanvasNodeType.Audio] || 0} 个，分组 ${counts[CanvasNodeType.Group] || 0} 个。`;
}
