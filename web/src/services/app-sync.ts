...
import type { CanvasDeletedProject, CanvasProject } from "@/app/(user)/canvas/stores/use-canvas-store";
...
function mergeCanvasData(local: CanvasDomainData, remote: CanvasDomainData): CanvasDomainData {
    ...
    return { projects, deleted: [...deletedAtById.entries()].map(([id, deletedAt]) => ({ id, deletedAt })) };
}
