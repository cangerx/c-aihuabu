...
| **AI 创作** | 默认经同域 Go 代理请求 OpenAI 兼容接口，也可切回浏览器直连；支持文生图、图生图、参考图编辑、文本问答、音视频生成 |
...
启动后访问 `http://localhost:3000`，进入右上角配置，填入自己的 OpenAI 兼容 `Base URL` 与 `API Key` 即可开始创作。Docker 部署默认使用“同域代理”规避上游 CORS；纯静态托管或渠道已开放 CORS 时可在“生成偏好”里切回“浏览器直连”。WebDAV 仍需要服务自身允许浏览器 CORS 请求。
